    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="glass-panel p-8 w-full max-w-md text-center space-y-6">
        ...
        <p className="text-slate-400">
          Real-time sprint planning with your team
        </p>
...
function RoomScreen({ onLeave }) {
  const { roomInfo, userId, userName, isMediator, participants } =
    useGameStore();
  const { castVote, leaveRoom } = useSupabaseRoom(
    roomInfo.id,
    userId,
    userName,
  );
      ...
      <div className="flex-1 min-h-0 flex gap-0 px-4 pb-0">
        <div className="flex-none w-48">
          <ParticipantSidebar
            ...
            onOpenTransferModal={
              isMediator ? () => setShowTransferModal(true) : undefined
            }
            groups={groups}
            participantGroupMap={participantGroupMap}
            onJoinGroup={joinGroup}
            onOpenGroupManager={
              isMediator ? () => setShowGroupManager(true) : undefined
            }
          />
        </div>
...
export default function App() {
  ...
  if (screen === "join")
    return <JoinRoom onJoined={handleRoomJoined} initialCode={joinCode} />;
