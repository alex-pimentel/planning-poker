    ...
    <div className="glass-panel p-4 flex flex-col min-h-0">
      <div className="flex items-center justify-between shrink-0">
        <h3 className="text-sm font-semibold text-slate-300">
          Tasks {tasks.length > 0 && <span className="text-slate-500 font-normal">({tasks.length})</span>}
        </h3>
        <button
      ...
      {!collapsed && (
        <div className="flex flex-col min-h-0 flex-1 mt-3">
          {isMediator && (
            <div className="flex gap-2 shrink-0">
              <input
                ...
                className="flex-1 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10
              ...
              <button onClick={handleAdd} className="glass-button text-sm px-3 py-1.5">
                Add
              </button>
            </div>
          )}
          ...
          {tasks.length > 0 && (
            <div className="space-y-1 overflow-y-auto max-h-52 mt-3 scrollbar-custom">
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all
                      ${isCurrent ? 'bg-poker-500/20 border border-poker-500/30' : isInitial ? 'bg-white/[0.07] border border-white/10' : 'bg-white/5 border border-white/5'}
                  ...
                  >
                    ...
                    {isEditing ? (
                      <input
                        ...
                        className="flex-1 px-2 py-0.5 rounded bg-white/10 text-white text-sm
                    ...
                    {isInitial && !isCurrent && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-white/10 text-slate-500 shrink-0">
                        Inicial
                      </span>
                    )}
                    ...
                    {isCurrent && (
                      <span className="text-[10px] text-poker-400 font-medium shrink-0">◉</span>
                    )}
          ...
          {tasks.length === 0 && isMediator && (
            <p className="text-xs text-slate-500 italic mt-3">No tasks yet. Add tasks to plan your sprint.</p>
          )}
        </div>
      )}
    </div>
