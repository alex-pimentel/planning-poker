      ...
      <div className="glass-panel p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto space-y-4">
        ...
        {isMediator && (
          <div className="flex gap-2">
            <input
              ...
              autoFocus
            />
            <button onClick={handleAdd} className="glass-button text-sm">Add Group</button>
          </div>
        )}

        {groups.length === 0 && (
          <p className="text-sm text-slate-400 italic">
            {isMediator ? 'Create your first group to organize participants.' : 'No groups have been created yet.'}
          </p>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {groups.map((group) => {
            ...
            return (
              <div key={group.id} className="bg-white/5 rounded-xl p-3 border border-white/10 space-y-2">
                <div className="flex items-center justify-between">
                  {isEditing ? (
                    <input
                      type="text"
